...
local Colors = assert(_G.ExwindGUIColor, "ExwindGUIColor.lua must load before ExwindUnifiedPanel.lua")
    ...
    for index, tab in ipairs(tabs or {}) do
        if type(tab) == "table" and tab.key and tab.label then
            ...
            if not button then
                ...
                button.hover:SetColorTexture(unpack(Colors.Control.Button.Secondary.HoverFill))
                button.hover:Hide()
...
local function SetRailButtonState(button, active)
    ...
    button:SetBackdropColor(unpack(active and Colors.Surface.PanelHeader or Colors.Transparent))
    button:SetBackdropBorderColor(unpack(active and accent or Colors.Transparent))
            ...
            if not button then
                ...
                button:SetScript("OnEnter", function(self)
                    if not self._active then
                        self:SetBackdropColor(unpack(Colors.Surface.PanelHeaderHover))
                    end
...
function Panel:CreateFrame()
    ...
    settingsBtn:SetBackdropColor(unpack(Colors.Transparent))
    settingsBtn:SetBackdropBorderColor(unpack(Colors.Transparent))
    ...
    changelog:SetBackdropColor(unpack(Colors.Transparent))
    changelog:SetBackdropBorderColor(unpack(Colors.Transparent))
    ...
    changelog:SetScript("OnEnter", function(self)
        self:SetBackdropColor(unpack(Colors.Surface.PanelHeaderHover))
        self:SetBackdropBorderColor(unpack(Colors.Accent.Primary))
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:SetText(L["更新日志"], 0.86, 0.92, 1)
        GameTooltip:Show()
    end)
    changelog:SetScript("OnLeave", function(self)
        self:SetBackdropColor(unpack(Colors.Transparent))
        self:SetBackdropBorderColor(unpack(Colors.Transparent))
        GameTooltip:Hide()
    end)
    ...
    editMode:SetBackdropColor(unpack(Colors.Transparent))
    editMode:SetBackdropBorderColor(unpack(Colors.Transparent))
    ...
    reload:SetBackdropColor(unpack(Colors.Transparent))
    reload:SetBackdropBorderColor(unpack(Colors.Transparent))
